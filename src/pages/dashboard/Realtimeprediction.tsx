...
const Index = () => {
  ...
  const fetchRealTimeData = async () => {
    ...
    try {
      ...
      console.log("Air Temperature:", parseFloat(latestEntry.field1).toFixed(0), "°C");
      console.log("Air Humidity:", sensorData.humidity, "%");
      console.log("Soil Temperature:", sensorData.soilTemperature, "°C");
  ...
  // Fetch moisture prediction from the new API
  const fetchMoisturePrediction = async (cropName: string) => {
    const apiUrl = 'https://moisture-future-endpoint.onrender.com/predict';
    ...
    const requestBody = {
      ...
      current_moisture: parseFloat(realTimeData.soilMoisture),
      city: manualInput.district,
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch moisture prediction: ${response.statusText}`);
      }

      const data = await response.json();
      console.log(`Moisture Prediction Advice for ${cropName}: ${data.Advice}`);
      return data.Advice;
    } catch (error) {
      console.error('Error fetching moisture prediction:', error);
      return 'Unable to fetch irrigation advice.';
    }
  };

  // Fetch fertilizer advice from the new API
  const fetchFertilizerAdvice = async (cropName: string) => {
    const apiUrl = 'https://crop-recommendation-gc3x.onrender.com/fertilizer_advice';
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch fertilizer advice: ${response.statusText}`);
      }

      const data = await response.json();
      console.log(`Fertilizer Advice for ${cropName}: ${data.advice}`);
      return data.advice;
    } catch (error) {
      console.error('Error fetching fertilizer advice:', error);
      return 'Unable to fetch fertilizer advice.';
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center space-x-2">
            {isHardwareConnected ? (
              <div className="flex items-center space-x-2 text-green-600">
                ...
                <span className="text-sm font-medium">Hardware Connected</span>
              </div>
            ) : (
              <div className="flex items-center space-x-2 text-red-600">
                <WifiOff className="w-4 h-4" />
                <span className="text-sm font-medium">Hardware Disconnected</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-gradient-to-r from-green-600 to-green-500 rounded-lg p-6 text-white">
          ...
          <p className="text-green-100">Connect hardware sensors and get AI-powered crop recommendations</p>
        </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <span className="text-sm font-medium text-green-800">pH Level</span>
                        </div>
                        <p className="text-2xl font-bold text-green-600">{realTimeData.ph}</p>
                      </div>
                      ...
                      <div className="bg-cyan-50 rounded-lg p-4 border border-cyan-200">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <span className="text-sm font-medium text-cyan-800">Soil Moisture</span>
                        </div>
                        <p className="text-2xl font-bold text-cyan-600">{realTimeData.soilMoisture}%</p>
                      </div>

                      <div className="bg-red-50 rounded-lg p-4 border border-red-200">
                        <div className="flex items-center space-x-2 mb-2">
                          <Thermometer className="w-5 h-5 text-red-500" />
                          <span className="text-sm font-medium text-red-800">Soil Temperature</span>
                        </div>
                        <p className="text-2xl font-bold text-red-600">{realTimeData.soilTemperature}°C</p>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                        <div className="flex items-center space-x-2 mb-2">
                          <BarChart className="w-5 h-5 text-green-500" />
                          <span className="text-sm font-medium text-green-800">Ideal pH</span>
                        </div>
                        <p className="text-2xl font-bold text-green-600">{idealParameters.ph}</p>
                      </div>
                ...
                <div className="bg-gradient-to-r from-purple-100 to-pink-100 rounded-lg p-6 border border-purple-200">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-2xl font-bold text-purple-800 mb-2">{idealCrop.name}</h3>
                      <div className="flex items-center space-x-4 mb-2">
                        <span className="text-purple-600 font-semibold">Suitability: {idealCrop.suitability}%</span>
                      ...
                      <p className="text-purple-700">{idealCrop.reason}</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        {cropPrediction && (
          <div className="grid grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-xl font-bold text-yellow-800">Expected Produced Crop</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="bg-yellow-50 rounded-lg p-6 border border-yellow-200">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <p className="text-3xl font-bold text-yellow-800">{cropPrediction.expectedProduce} tons</p>
                      <p className="text-yellow-600">Per hectare production</p>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-xl font-bold text-green-800">Estimated Income</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="bg-green-50 rounded-lg p-6 border border-green-200">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <p className="text-3xl font-bold text-green-800">₹{parseInt(cropPrediction.estimatedIncome).toLocaleString()}</p>
                      <p className="text-green-600">Expected total income</p>
                    </div>
                  </div>
                </div>
              </CardContent>
